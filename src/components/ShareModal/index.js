...
class ShareModal extends Component {
    ...
    searchFieldChange(field) {
        this._fieldStr = field.toString();
        if (this._fieldStr.length > 0) {
            const { homePageList } = this.props;
            const homePageListCopy = [...List(homePageListCopy)];
            const fuse = new Fuse(homePageListCopy, this.filterOptions)
            const contactedItems = fuse.search(this._fieldStr);
            this.setState({
                isSearching: true,
                contactedItems
            })
